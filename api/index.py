...
# --- Flask App Setup ---
# This is the application object that Vercel will run.
app = Flask(__name__)
CORS(app)
...
def query_articles(filters):
    ...
    try:
        ...
        if filters.get('search'):
            ...
            search_clause = "(LOWER(original_title) LIKE ? OR LOWER(llm_generated_title) LIKE ? OR LOWER(article_description) LIKE ? OR LOWER(source) LIKE ? OR LOWER(article_content) LIKE ?)"
        ...
        for row in rows:
            ...
            if not article_dict.get(IMAGE_COLUMN_NAME) or not isinstance(article_dict.get(IMAGE_COLUMN_NAME), str) or not article_dict.get(IMAGE_COLUMN_NAME).startswith('http'):
...
def get_articles():
    ...
    filters = {
        ...
        'month': request.args.get('month', default="All Months", type=str),
        'day': request.args.get('day', default="All Days", type=str),
        'category': request.args.get('category', default="All Categories", type=str),
    }
